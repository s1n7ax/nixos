...
Dev tooling runs natively on macOS: the `macbook` profile imports the same
home-manager tree as the Linux profiles and enables the feature set in
`profile/macbook/host-options.nix` (neovim, fish, kitty, git/gh/acli, the
language toolchains, CLI utilities). There is no `linux-builder` VM — Docker is
provided by [colima](https://github.com/abiosoft/colima), started by
home-manager when `features.virtualization.docker` is on.

Secrets are not available on macOS: the `secrets` flake input and the sops
modules are imported only by `profile/common`, which the macbook profile does
not use.

Convenience aliases (see `system/home-manager/applications/alias.nix`, Darwin
half):

```shell
mac-update    # cd ~/nixos && nix flake update
mac-rebuild   # darwin-rebuild switch --flake ~/nixos#macbook
mac-clean     # garbage-collect and optimise the store
```
