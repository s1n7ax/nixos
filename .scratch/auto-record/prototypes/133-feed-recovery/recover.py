#!/usr/bin/env python3
"""Throwaway rig for wayfinder issue #133, forked from #130's deadfeed.py.

#130 proved the pause and the camera's resume, and diagnosed the other two
feeds without recovering either. This rig recovers them:

1. The mic. `pipewiresrc` never reconnects, so the audio source is torn out of
   the running pipeline and a fresh one put in its place, selected by PipeWire
   *serial* - a serial that changes every time the node reappears.

2. The desktop. A fresh portal handshake on top of the held session yields a
   new fd and node id, which are swapped into a fresh `pipewiresrc` the same
   way. The restore token is persisted so a later run of the program skips the
   share picker.
...
class Rig:
    ...
    def on_error(self, _bus, msg):
        ...
        feed = {"screen": "screen", "cam": "camera", "mic": "mic"}.get(src)
        if feed:
